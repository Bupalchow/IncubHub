import { SigningScheme as A, Secp256k1PublicKey as d, Secp256k1Signature as v, AccountAuthenticatorSingleKey as m, AnyPublicKey as p, AnySignature as y } from "@aptos-labs/ts-sdk";
import { APTOS_CHAINS as w, UserResponseStatus as r, registerWallet as P } from "@aptos-labs/wallet-standard";
class b {
  constructor(a) {
    this.address = a.accountAddress.toString(), this.publicKey = a.publicKey.toUint8Array(), this.chains = w, this.features = ["aptos:connect"], this.signingScheme = A.SingleKey;
  }
}
class u {
  constructor(a = "PROD") {
    this.env = a, this.url = "https://www.twallet.ai", this.version = "1.0.0", this.name = "T wallet", this.icon = "data:image/png;base64,", this.chains = w, this.accounts = [], this.provider = typeof window < "u" ? window.dekey : void 0, this.initializing = !1, this.account = async () => {
      var t;
      const e = await ((t = this.provider) == null ? void 0 : t.request({
        method: "aptos_account"
      }));
      return {
        address: e.address,
        publicKey: e.publicKey
      };
    }, this.connect = async () => {
      var s;
      if (!this.initializing) {
        this.initializing = !0, await import(
          // @ts-ignore
          "./index-BIfmAjZ-.mjs"
        );
        const i = D(this.env);
        if (!i)
          throw this.initializing = !1, new Error(`walletDomain not found with env: ${this.env}`);
        if (!(window != null && window.initializeDekeyProvider))
          throw this.initializing = !1, new Error("initializeDekeyProvider not found");
        await window.initializeDekeyProvider(i), this.provider = window.dekey;
      }
      if (!await this.checkProvider())
        return {
          status: r.REJECTED
        };
      const t = await ((s = this.provider) == null ? void 0 : s.request({
        method: "aptos_requestAccounts"
      }));
      return t ? {
        status: r.APPROVED,
        args: {
          address: t.address,
          publicKey: t.publicKey
        }
      } : (await new Promise((i) => setTimeout(i, 2e3)), {
        status: r.REJECTED
      });
    }, this.checkProvider = async () => new Promise((e) => {
      setTimeout(() => {
        e(!1);
      }, 1e4);
      const t = setInterval(() => {
        this.provider && (e(!0), clearInterval(t));
      }, 100);
    }), this.network = async () => {
      const { chainId: e, name: t, url: s } = await this.provider.request({
        method: "aptos_network"
      });
      return {
        name: t,
        chainId: e,
        url: s
      };
    }, this.disconnect = async () => Promise.resolve(), this.signTransaction = async (e, t) => {
      try {
        const s = e.rawTransaction.bcsToBytes(), { concatedSig: i, uncompressedPubkey: c } = await this.provider.request({
          method: "aptos_signTransaction",
          params: [Array.from(s)]
        }), h = new d(c), g = new v(i), l = new m(
          new p(h),
          new y(g)
        );
        return Promise.resolve({
          status: r.APPROVED,
          args: l
        });
      } catch (s) {
        throw s.message;
      }
    }, this.signAndSubmitTransaction = async (e) => {
      const t = e.rawTransaction.bcsToBytes(), s = await this.provider.request({
        method: "aptos_signAndSubmitTransaction",
        params: [Array.from(t)]
      });
      return {
        status: r.APPROVED,
        args: s
      };
    }, this.signMessage = async (e) => {
      var t;
      try {
        const s = await ((t = this.provider) == null ? void 0 : t.request({
          method: "aptos_signMessage",
          params: [e.message]
        }));
        return {
          status: r.APPROVED,
          args: s
        };
      } catch (s) {
        throw s.message;
      }
    }, this.onAccountChange = async () => Promise.resolve(), this.onNetworkChange = async (e) => {
      const t = async () => {
        const s = await this.network(), { name: i, chainId: c, url: h } = s;
        e({
          name: i,
          chainId: c,
          url: h
        });
      };
      this.provider.on("chainChanged", () => {
        t();
      });
    }, a === "PROD" && o(this);
  }
  get features() {
    return {
      "aptos:connect": {
        version: "1.0.0",
        connect: this.connect
      },
      "aptos:network": {
        version: "1.0.0",
        network: this.network
      },
      "aptos:disconnect": {
        version: "1.0.0",
        disconnect: this.disconnect
      },
      "aptos:signTransaction": {
        version: "1.0.0",
        signTransaction: this.signTransaction
      },
      "aptos:signAndSubmitTransaction": {
        version: "1.0.0",
        signAndSubmitTransaction: this.signAndSubmitTransaction
      },
      "aptos:signMessage": {
        version: "1.0.0",
        signMessage: this.signMessage
      },
      "aptos:onAccountChange": {
        version: "1.0.0",
        onAccountChange: this.onAccountChange
      },
      "aptos:onNetworkChange": {
        version: "1.0.0",
        onNetworkChange: this.onNetworkChange
      },
      "aptos:account": {
        version: "1.0.0",
        account: this.account
      }
    };
  }
}
const D = (n) => {
  if (n === "LOCAL")
    return "http://localhost:5173";
  if (n === "DEV")
    return "https://dev.twallet.ai";
  if (n === "QA")
    return "https://qa.twallet.ai";
  if (n === "PROD")
    return "https://www.twallet.ai";
}, o = (n) => {
  typeof window > "u" || P(n);
};
class C extends u {
  constructor() {
    super("LOCAL"), this.url = "http://localhost:5173", this.name = "Local T wallet", o(this);
  }
}
class F extends u {
  constructor() {
    super("DEV"), this.url = "https://dev.twallet.ai", this.name = "Dev T wallet", o(this);
  }
}
class U extends u {
  constructor() {
    super("QA"), this.url = "https://qa.twallet.ai", this.name = "Qa T wallet", o(this);
  }
}
export {
  F as DevTWallet,
  C as LocalTWallet,
  b as MyWalletAccount,
  U as QaTWallet,
  u as TWallet
};
